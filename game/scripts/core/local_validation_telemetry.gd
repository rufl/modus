...
func start_session(p_session_id: String = "") -> String:
	...
	if p_session_id.is_empty():
		...
		p_session_id = "validation_%04d%02d%02d_%02d%02d%02d" % [
			datetime.year,
			datetime.month,
			datetime.day,
			datetime.hour,
...
func stop_session() -> void:
	if _log_file == null:
		return
	record_event("session_stopped", {"duration_seconds": (Time.get_ticks_msec() / 1000.0) - _started_at})
	_log_file.close()
	_log_file = null
	set_process(false)
...
func _on_log_emitted(entry: Dictionary) -> void:
	...
	if level in ["[WARNING]", "[ERROR]"]:
		record_event("log_signal", {"level": level, "context": entry.get("context", ""), "message": entry.get("message", "")})
