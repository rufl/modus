...
## Test runner for MultiMesh Manager tests
## Usage: godot --headless --script game/scripts/map_generator/tests/run_multimesh_tests.gd
...
func _init() -> void:
	print("\n" + "=".repeat(70))
	print("  MULTIMESH MANAGER TEST SUITE")
	print("  Testing: MultiMesh batching for repeated prefabs")
	print("=".repeat(70) + "\n")
	...
	# Add test file
	_gut.add_script("res://game/scripts/map_generator/tests/test_multimesh_manager.gd")
...
func _on_tests_finished() -> void:
	...
	if exit_code == 0:
		print("✓ All MultiMesh tests passed!")
	else:
