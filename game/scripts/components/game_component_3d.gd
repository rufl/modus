## Base class for 3D game components with automatic signal hygiene
## Provides safe_connect() for automatic signal cleanup and memory leak prevention
class_name GameComponent3D
extends Node3D
...
func safe_connect(sig: Signal, callable: Callable, flags: int = 0) -> void:
	if not sig.get_object():
		push_error("GameComponent3D.safe_connect: Invalid signal (no object)")
		return

	# Check if already connected to avoid duplicate connections
	if sig.is_connected(callable):
		push_warning("GameComponent3D.safe_connect: Signal already connected, skipping")
		return
