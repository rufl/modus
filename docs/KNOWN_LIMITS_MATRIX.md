...
| Editor | Focused save/export/reload, runtime history, `.mdsl` export, and local Workshop simulation contracts pass | Native exported-app/full graphical UI operation and real Workshop transfer remain open | `EDITOR_ROUNDTRIP_PROOF.md`, `WORKSHOP_LOCAL_SIMULATION_PROOF.md` |
...
| Distribution | Windows client, Windows standalone editor, and Linux dedicated-server executables export successfully with installed Godot 4.7.2 templates; all three resource exports pass six required notice/ledger checks byte-for-byte with no local logs or generated ledger translations | Provenance ledger is fully cleared; installer definition/toolchain is absent; executable runtime testing on target Windows hardware remains open | `ATTRIBUTION.md`, `PROVENANCE_LEDGER.csv`, `../tools/validate_production_readiness.sh` |
