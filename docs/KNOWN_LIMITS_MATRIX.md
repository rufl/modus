...
| Multiplayer | Local real-ENet host lifecycle and separate-process localhost client connection pass; reconnect token/cancellation is 2/2; controlled 100 ms combat rewind is 10/10; network-manager/rate validation is 13/13; profile launch is clean | Real Steam/GodotSteam, representative WAN sessions, real abusive-client soak, end-to-end automatic reconnect across a live service, and dedicated-server executable clients remain unproven; executable templates are unavailable in this environment | `../tools/run_enet_local_smoke.sh`, `../tools/run_multiplayer_profile_smoke.sh`, `../tests/unit/test_player_reconnect.gd`, `../tests/unit/test_combat_lag_integration.gd`, `../tests/unit/test_network_manager.gd` |
...
| Distribution | Windows Desktop, Dedicated Server (Linux), and Standalone Editor resource exports each pass six required notice/ledger files byte-for-byte with no local logs or generated ledger translations | Twelve identified music tracks and 200 unverified assets remain uncleared; runnable executable exports are blocked by missing Godot Linux/Windows templates; no installer tooling exists in the repository | `ATTRIBUTION.md`, `PROVENANCE_LEDGER.csv`, `../tests/runners/test_export_notices.sh`, `../export_presets.cfg` |
