...
Focused network-manager, whitelist, movement-sync, and multiplayer compatibility tests exist, but the retained full suite is non-green. The profile startup smoke passes; current two-process ENet connectivity is blocked by sandbox socket policy; real Steam/GodotSteam is unproven.
