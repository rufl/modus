...
- Standalone block, erase, paint, transform, and selection history: **FOCUSED PASS**; runtime-safe fallback and live node-tree/material/transform selection round trips are covered. Full editor UI and manual authoring workflow remain open.
- Live complete editor UI workflow: **UNPROVEN**.
- Standalone custom undo/redo commands: **PARTIALLY OPEN**; paste/duplicate and manual authoring paths still require user-visible exercise.
