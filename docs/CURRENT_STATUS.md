...
| Network manager | 15/15; trusted Steam-authenticated peers remain subject to whitelist, rate limits, movement validation, semantic status/chat/kill/ticket/revive/interaction validation, authority-only damage, weapon visual effects, player-state synchronization, bounded lifecycle, grenade, prop-damage, chest, and backpack requests |
