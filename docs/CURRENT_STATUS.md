...
- Standalone block, erase, paint, transform, selection, and paste history: **FOCUSED PASS**; runtime-safe fallback, centroid-preserving paste, and live node-tree/material/transform round trips are covered. Full editor UI and manual authoring workflow remain open.
- Live complete editor UI workflow: **UNPROVEN**.
- Standalone custom undo/redo commands: **PARTIALLY OPEN**; duplicate and manual authoring paths still require user-visible exercise.
