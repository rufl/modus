...
func _run_stress_test() -> void:
	...
	print("[RPC Stress Test] Sending %d RPCs in 1 second (exceeds limit of %.1f)..." % [attempts, rate_limit])
	...
	print("[RPC Stress Test] Violations detected: %d (expected: ~%d)" % [violations, expected_violations])
	...
	test_results.test_passed = violations_correct and test_results.player_kicked and test_results.server_stable


func _simulate_rpc_call(rpc_name: String, rate_limit: float, call_index: int, total_calls: int) -> bool:
...
class IntegrationTest extends Node:
	var network_manager: Node = null
