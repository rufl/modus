...
const VALID_TRANSITIONS = {
	# INITIALIZING -> READY or SHUTTING_DOWN
	0: [1, 4],
	# READY -> RUNNING or SHUTTING_DOWN
	1: [2, 4],
	# RUNNING -> PAUSED or SHUTTING_DOWN
	2: [3, 4],
	# PAUSED -> RUNNING or SHUTTING_DOWN
	3: [2, 4],
	# SHUTTING_DOWN -> (terminal state)
	4: []
}
...
func _test_state_changed_signal(test_data: Dictionary) -> bool:
	...
	var result = (
		signal_state.emitted and signal_state.old == old_state and signal_state.new == new_state
	)
...
func _test_same_state_no_signal(test_data: Dictionary) -> bool:
	...
	# Track signal emissions
	var signal_count = 0
	gm.state_changed.connect(func(_old, _new): signal_count += 1)
