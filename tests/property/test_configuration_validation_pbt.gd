...
func _test_invalid_json5_syntax(test_data: Dictionary) -> bool:
	...
	config_mgr.config_error.connect(
		func(file_path: String, msg: String) -> void:
	...
	var test_passed: bool = (
		error_state["emitted"]
		and result.is_empty()
		and (error_state["file_path"] as String).contains(test_file_path)
	)
...
func _test_missing_required_fields(test_data: Dictionary) -> bool:
	...
	var config := {
		...
		"field2": "test_value",
		"nested": {"field3": rng.randf_range(0.0, 1.0)}
	}
...
func _test_valid_configuration(test_data: Dictionary) -> bool:
	...
	var config := {
		...
		"nested": {"nested_int": rng.randi_range(1, 100)}
	}
...
func _test_error_signal_file_path(test_data: Dictionary) -> bool:
	...
	config_mgr.config_error.connect(
		func(file_path: String, _msg: String) -> void:
			error_state["emitted"] = true
			error_state["file_path"] = file_path
	)
...
func _test_nested_field_validation(test_data: Dictionary) -> bool:
	...
	var schema := {schema_path: TYPE_INT}
