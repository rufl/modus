...
func test_property_player_count_consistency() -> void:
	...
	for i in range(10):
		...
		assert_eq(
			session_state.player_count,
			session_state.active_players.size(),
...
func test_property_unique_player_ids() -> void:
	...
	for player_id: int in session_state.active_players.keys():
		assert_false(unique_ids.has(player_id), "Player ID %d must be unique" % player_id)
...
func test_property_gamepad_assignment_uniqueness() -> void:
	...
	for player_id: int in gamepad_controller._player_to_device.keys():
		...
		assert_false(
			assigned_devices.has(device_id),
...
func test_property_bidirectional_mapping_consistency() -> void:
	...
	for player_id: int in gamepad_controller._player_to_device.keys():
		...
		assert_eq(player_id, reverse_player, "Bidirectional mapping must be consistent")
...
func test_property_viewport_count_equals_player_count() -> void:
	...
	assert_eq(
		viewport_manager.get_viewport_count(),
		6,
...
func test_property_viewport_area_coverage() -> void:
	for player_count: int in [4, 5, 6]:
		...
		assert_almost_eq(
			total_area, 1.0, 0.01, "Total viewport area must be 100%% for %d players" % player_count
		)
...
func test_property_no_viewport_overlap() -> void:
	...
	for i in range(rects.size()):
		for j in range(i + 1, rects.size()):
			...
			assert_false(overlap, "Viewports %d and %d must not overlap" % [i, j])
...
func test_property_state_transition_atomicity() -> void:
	...
	assert_true(
		session_state.current_state == initial_state or session_state.current_state == target_state,
...
func test_property_player_removal_cleanup() -> void:
	...
	assert_false(session_state.active_players.has(0), "Player must be removed from active_players")
	assert_false(session_state.gamepad_assignments.has(0), "Gamepad assignment must be removed")
...
func test_property_session_timer_monotonicity() -> void:
	...
	for i in range(10):
		...
		assert_ge(current_time, prev_time, "Session time must never decrease")
...
func test_property_input_isolation() -> void:
	...
	assert_eq(input_comp1.get_device_id(), 0, "Component 1 must only see device 0")
	assert_eq(input_comp2.get_device_id(), 1, "Component 2 must only see device 1")
...
func test_property_gamepad_reconnection_preservation() -> void:
	...
	assert_eq(
		gamepad_controller.get_player_for_device(0),
		5,
...
func test_property_layout_type_determinism() -> void:
	for player_count: int in [4, 5, 6]:
		...
		assert_eq(
			layout1, layout2, "Layout type must be deterministic for player count %d" % player_count
		)
...
func test_property_signal_emission_guarantee() -> void:
	...
	assert_signal_emitted(
		gamepad_controller, "gamepad_assigned", "assign_gamepad must emit gamepad_assigned signal"
	)
...
func test_property_configuration_idempotence() -> void:
	var config: Dictionary = {"vibration_enabled": false, "analog_deadzone": 0.25}
...
func test_property_fps_sample_bounded_size() -> void:
	...
	assert_le(
		splitscreen_manager._fps_samples.size(),
		60,
...
func test_property_assignment_completion_correctness() -> void:
	...
	# Not complete yet
	assert_false(
		gamepad_controller.is_assignment_complete(), "Assignment not complete with 0/4 players"
	)
	...
	assert_false(
		gamepad_controller.is_assignment_complete(), "Assignment not complete with 3/4 players"
	)

	# Assign 4th player
	gamepad_controller.assign_gamepad(3, 3)

	assert_true(gamepad_controller.is_assignment_complete(), "Assignment complete with 4/4 players")
...
func test_property_state_serialization_completeness() -> void:
	...
	for key: String in required_keys:
		assert_true(dict.has(key), "Serialized state must include %s" % key)
...
func test_property_quality_adjustment_stability() -> void:
	...
	assert_eq(initial_quality, final_quality, "Quality must remain stable with stable FPS")
...
func test_property_device_detection_consistency() -> void:
	...
	assert_eq(
		detected.size(),
		gamepad_controller._connected_devices.size(),
...
func test_property_viewport_equal_area_distribution() -> void:
	for player_count: int in [4, 5, 6]:
		...
		for rect: Rect2 in rects:
			...
			assert_almost_eq(
				area, expected_area, 0.01, "Each viewport must have area ≈ 1/%d" % player_count
			)
