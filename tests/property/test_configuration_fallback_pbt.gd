...
func _test_fallback_missing_key(test_data: Dictionary) -> bool:
	...
	var config := {
		"existing_key": rng.randi_range(1, 100),
		"nested": {
...
func _test_fallback_invalid_json5(test_data: Dictionary) -> bool:
	...
	var test_file_path := TEST_CONFIG_DIR + "test_fallback_invalid_%d.json5" % test_data.get("iteration", 0)
...
func _test_fallback_nested_missing(test_data: Dictionary) -> bool:
	...
	var config := {
		"level1": {
			"level2": {
				"existing_value": rng.randi_range(1, 100)
			}
		}
	}
	...
	for i in range(operations_count):
		...
		match operation_type:
			...
			2:  # Try to validate invalid config
				...
				assert_property_push_error("Validation failed", "Invalid configuration should produce one diagnostic")
...
func _test_fallback_partial_config(test_data: Dictionary) -> bool:
	...
	var config := {
		...
		"nested": {
			"valid_nested": rng.randf_range(0.0, 1.0)
		}
	}
