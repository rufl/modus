...
func test_initialization_creates_components() -> void:
	assert_not_null(splitscreen_manager.viewport_manager, "Should create ViewportManager")
	assert_not_null(splitscreen_manager.gamepad_controller, "Should create GamepadController")
	assert_not_null(splitscreen_manager.session_state, "Should create SessionState")


## Test: Initial state is inactive
func test_initial_state_inactive() -> void:
	assert_false(splitscreen_manager.is_session_active(), "Session should not be active initially")
	assert_eq(splitscreen_manager.get_player_count(), 0, "Player count should be 0 initially")
...
func test_session_started_signal() -> void:
	...
	assert_signal_emitted(
		splitscreen_manager, "session_started", "Should emit session_started signal"
	)
...
func test_end_session_cleanup() -> void:
	...
	assert_false(splitscreen_manager.is_session_active(), "Session should not be active after end")
	assert_eq(splitscreen_manager.get_player_count(), 0, "Player count should be 0 after end")
...
func test_session_ended_signal() -> void:
	...
	assert_signal_emitted(splitscreen_manager, "session_ended", "Should emit session_ended signal")
...
func test_add_player() -> void:
	...
	assert_eq(splitscreen_manager.get_player_count(), 1, "Player count should be 1")
...
func test_player_joined_signal() -> void:
	...
	assert_signal_emitted(splitscreen_manager, "player_joined", "Should emit player_joined signal")
...
func test_remove_player() -> void:
	...
	assert_eq(splitscreen_manager.get_player_count(), 0, "Player count should be 0 after removal")
...
func test_player_left_signal() -> void:
	...
	assert_signal_emitted(splitscreen_manager, "player_left", "Should emit player_left signal")
...
func test_get_session_state() -> void:
	...
	assert_eq(
		state, splitscreen_manager.session_state, "Should return the correct session state instance"
	)


## Test: Is session active check
func test_is_session_active() -> void:
	assert_false(splitscreen_manager.is_session_active(), "Should not be active initially")

	_transition_session_to_active()

	assert_true(splitscreen_manager.is_session_active(), "Should be active after transition")


## Test: Get player count
func test_get_player_count() -> void:
	assert_eq(splitscreen_manager.get_player_count(), 0, "Should be 0 initially")

	splitscreen_manager.session_state.add_player(0)
	splitscreen_manager.session_state.add_player(1)

	assert_eq(splitscreen_manager.get_player_count(), 2, "Should be 2 after adding players")
...
func test_gamepad_disconnection_updates_player_status() -> void:
	...
	assert_false(player_data.connected, "Player should be marked as disconnected")
...
func test_cleanup_on_error() -> void:
	...
	assert_eq(
		splitscreen_manager.viewport_manager.get_viewport_count(), 0, "Should cleanup viewports"
	)
	assert_eq(
		splitscreen_manager.gamepad_controller.get_assigned_players().size(),
		0,
		"Should clear gamepad assignments"
	)
	assert_eq(splitscreen_manager.session_state.player_count, 0, "Should reset session state")
...
func test_set_player_scene() -> void:
	...
	assert_eq(splitscreen_manager.player_scene, mock_scene, "Should set player scene")


## Test: Configuration loading
func test_configuration_loading() -> void:
	# Configuration should be loaded during initialize()
	assert_eq(splitscreen_manager.min_players, 4, "Should load min_players from config")
	assert_eq(splitscreen_manager.max_players, 6, "Should load max_players from config")
	assert_eq(
		splitscreen_manager.default_player_count,
		4,
...
func test_performance_monitoring() -> void:
	...
	assert_gt(splitscreen_manager._fps_samples.size(), 0, "Should collect FPS samples")
...
func test_quality_adjustment_on_low_fps() -> void:
	...
	splitscreen_manager._last_quality_adjustment_time = (
		splitscreen_manager.quality_adjustment_interval
	)
	splitscreen_manager._monitor_performance(0.016)

	# Quality should be adjusted down
	assert_ne(
		splitscreen_manager._current_quality_level, "high", "Quality should be reduced on low FPS"
	)
