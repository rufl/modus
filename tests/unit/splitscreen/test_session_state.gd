...
func test_initial_state_is_inactive() -> void:
	assert_eq(session_state.current_state, SessionState.State.INACTIVE,
		"Initial state should be INACTIVE")
	assert_eq(session_state.player_count, 0,
		"Initial player count should be 0")
	assert_eq(session_state.active_players.size(), 0,
		"Initial active players should be empty")
...
func test_valid_transition_inactive_to_initializing() -> void:
	...
	assert_eq(session_state.current_state, SessionState.State.INITIALIZING,
...
func test_valid_transition_initializing_to_assigning_devices() -> void:
	...
	assert_true(result, "Transition should succeed")
	assert_eq(session_state.current_state, SessionState.State.ASSIGNING_DEVICES,
...
func test_valid_transition_assigning_devices_to_active() -> void:
	...
	assert_true(result, "Transition should succeed")
	assert_eq(session_state.current_state, SessionState.State.ACTIVE,
...
func test_valid_transition_active_to_paused() -> void:
	...
	assert_true(result, "Transition should succeed")
	assert_eq(session_state.current_state, SessionState.State.PAUSED,
...
func test_valid_transition_active_to_ending() -> void:
	...
	assert_true(result, "Transition should succeed")
	assert_eq(session_state.current_state, SessionState.State.ENDING,
		"State should be ENDING")
...
func test_session_timer_update() -> void:
	...
	assert_almost_eq(session_state.total_session_time, 1.0, 0.01, 
		"Total session time should be ~1.0 seconds")
...
func test_performance_metrics_update() -> void:
	...
	assert_almost_eq(session_state.get_performance_metric("frame_time"), 16.67, 0.01,
		"Frame time should be ~16.67")
	assert_eq(session_state.get_performance_metric("memory_usage"), 1024,
		"Memory usage should be 1024")
