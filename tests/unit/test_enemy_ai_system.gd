...
func test_ai_controller_has_initial_state() -> void:
	...
	assert_not_null(ai_controller.current_state, "AI controller should have current state set")
...
func test_ai_controller_can_change_state() -> void:
	...
	assert_eq(
		ai_controller.current_state, chase_state, "AI controller should change to chase state"
	)
...
func test_tier_2_veteran_behavior() -> void:
	...
	if health_component:
		assert_gt(
			health_component.max_health, 50.0, "Tier 2 enemies should have more health than tier 1"
		)
...
func test_tier_3_elite_behavior() -> void:
	...
	if squad_tactics:
		assert_true(true, "Tier 3 enemies can have squad tactics")
...
func test_tier_4_boss_behavior() -> void:
	...
	if health_component:
		assert_gt(health_component.max_health, 100.0, "Tier 4 bosses should have high health")
...
func test_aggression_levels() -> void:
	...
	if grunt_data.has("stats"):
		...
		assert_true(stats.has("aggression"), "Enemy data should have aggression value")
...
func test_tier_colors_defined() -> void:
	...
	for tier_num: int in range(1, 5):
		assert_true(tier_colors.has(tier_num), "Tier %d should have color defined" % tier_num)
...
func test_tier_data_consistency() -> void:
	...
	var tier_examples: Dictionary = {1: "grunt", 2: "healer", 3: "summoner", 4: "warlord"}
	...
	for expected_tier: int in tier_examples.keys():
		...
		assert_true(enemy_data.has("tier"), "%s should have tier defined" % enemy_id)

		if enemy_data.has("tier"):
			assert_eq(
				enemy_data.tier, expected_tier, "%s should be tier %d" % [enemy_id, expected_tier]
			)
...
func test_tier_loot_table_mapping() -> void:
	...
	var tier_loot_mapping: Dictionary = {
		1: "tier1_loot", 2: "tier2_loot", 3: "tier3_loot", 4: "tier4_boss_loot"
	}
	...
	for tier_num: int in tier_loot_mapping.keys():
		...
		assert_true(true, "Tier %d should map to %s" % [tier_num, expected_loot])
...
func test_tier_perception_scaling() -> void:
	...
	if perception_component.has_method("configure_from_data"):
		...
		assert_gte(
			tier3_radius, tier1_radius, "Tier 3 should have equal or better perception than tier 1"
		)
...
func test_tier_combat_effectiveness() -> void:
	...
	if "attack_damage" in combat_component:
		assert_gte(combat_component.attack_damage, 0.0, "Combat damage should be non-negative")
...
func test_tier_ai_complexity() -> void:
	...
	assert_gt(state_count, 0, "AI controller should have at least one state")
...
func test_tier_assignment_from_data() -> void:
	...
	assert_true(
		test_enemy.tier >= 1 and test_enemy.tier <= 4, "Enemy tier should be between 1 and 4"
	)
...
func test_tier_display_name_construction() -> void:
	...
	assert_true(
		tier_prefix == "tier_elite" or tier_prefix == "", "Tier 3 should have elite prefix or empty"
	)
...
func test_squad_tactics_component_creation() -> void:
	...
	assert_not_null(
		enemy.get_node_or_null("SquadTactics"), "Squad tactics component should be created"
	)
...
func test_squad_member_joining() -> void:
	...
	# Verify joined
	assert_eq(member_tactics.squad_leader, enemy, "Member should have leader set")
	assert_false(member_tactics.is_squad_leader, "Member should not be squad leader")
...
func test_squad_formation_types() -> void:
	...
	for formation: int in formations:
		...
		assert_eq(
			squad_tactics.current_formation, formation, "Squad should set formation: %d" % formation
		)
...
func test_squad_disbanding() -> void:
	...
	assert_false(squad_tactics.is_squad_leader, "Enemy should no longer be squad leader")
	assert_eq(squad_tactics.squad_id, "", "Squad ID should be cleared")
...
func test_squad_size_tracking() -> void:
	...
	assert_eq(squad_tactics.get_squad_size(), 1, "Squad should have 1 member initially")
...
func test_squad_center_calculation() -> void:
	...
	assert_almost_eq(center.x, 10.0, 0.1, "Squad center X should be at leader position")
	assert_almost_eq(center.z, 10.0, 0.1, "Squad center Z should be at leader position")
...
func test_squad_center_with_multiple_members() -> void:
	...
	var center: Vector3 = squad_tactics.get_squad_center()
	assert_almost_eq(center.x, 5.0, 0.1, "Squad center should be midpoint of members")
...
func test_squad_flanking_detection() -> void:
	...
	assert_false(should_flank, "Squad with 1 member should not flank")
...
func test_squad_flanking_with_enough_members() -> void:
	...
	assert_gte(
		squad_tactics.get_squad_size(), 3, "Squad should have at least 3 members for flanking"
	)
...
func test_squad_flank_position_calculation() -> void:
	...
	assert_ne(left_flank, Vector3.ZERO, "Left flank position should be calculated")
	assert_ne(right_flank, Vector3.ZERO, "Right flank position should be calculated")
	assert_ne(left_flank, right_flank, "Left and right flank positions should be different")
...
func test_squad_formation_position_retrieval() -> void:
	...
	assert_true(true, "Squad should provide formation position")
...
func test_squad_signals_formation_changed() -> void:
	...
	assert_signal_emitted(squad_tactics, "formation_changed", "Formation change should emit signal")
...
func test_squad_signals_squad_formed() -> void:
	...
	assert_true(
		squad_tactics.has_signal("squad_formed"), "Squad tactics should have squad_formed signal"
	)
...
func test_squad_signals_squad_disbanded() -> void:
	...
	# Signal should be emitted
	assert_signal_emitted(squad_tactics, "squad_disbanded", "Squad disbanding should emit signal")
...
func test_squad_formation_spacing_configuration() -> void:
	...
	assert_eq(squad_tactics.formation_spacing, 5.0, "Formation spacing should be configurable")
...
func test_squad_auto_join_configuration() -> void:
	...
	assert_false(squad_tactics.auto_join_squads, "Auto-join should be configurable")
...
func test_squad_search_radius_configuration() -> void:
	...
	assert_eq(squad_tactics.squad_search_radius, 30.0, "Squad search radius should be configurable")
...
func test_squad_non_leader_cannot_issue_orders() -> void:
	...
	assert_true(true, "Non-leaders should handle order attempts gracefully")
...
func test_squad_id_generation() -> void:
	...
	# Initially empty
	assert_eq(squad_tactics.squad_id, "", "Squad ID should be empty initially")

	# Form squad
	squad_tactics.try_form_squad()

	# Should generate ID
	assert_ne(squad_tactics.squad_id, "", "Squad ID should be generated when forming squad")
	assert_true(squad_tactics.squad_id.begins_with("squad_"), "Squad ID should have proper prefix")
...
func test_infighting_system_creation() -> void:
	...
	assert_not_null(
		enemy.get_node_or_null("InfightingSystem"), "Infighting system should be created"
	)
...
func test_infighting_enabled_by_default() -> void:
	...
	assert_true(infighting_system.enable_infighting, "Infighting should be enabled by default")
...
func test_infighting_retaliation_chance() -> void:
	...
	assert_gte(infighting_system.retaliation_chance, 0.0, "Retaliation chance should be >= 0")
	assert_lte(infighting_system.retaliation_chance, 1.0, "Retaliation chance should be <= 1")
...
func test_infighting_same_tier() -> void:
	...
	assert_true(infighting_system.is_infighting(), "Same tier infighting should start when enabled")
	assert_eq(
		infighting_system.get_infight_target(), attacker, "Infight target should be set to attacker"
	)
...
func test_infighting_duration() -> void:
	...
	assert_gt(infighting_system.infight_duration, 0.0, "Infighting duration should be positive")
...
func test_infighting_status_check() -> void:
	...
	assert_false(infighting_system.is_infighting(), "Enemy should not be infighting initially")
...
func test_infighting_lower_tier_attacks_higher() -> void:
	...
	assert_false(
		infighting_system.is_infighting(), "Lower tier should not attack higher tier when disabled"
	)
...
func test_infighting_higher_tier_attacks_lower() -> void:
	...
	assert_true(
		infighting_system.is_infighting(), "Higher tier should attack lower tier when enabled"
	)
...
func test_infighting_signals_started() -> void:
	...
	assert_signal_emitted(
		infighting_system, "infight_started", "Infighting start should emit signal"
	)
...
func test_infighting_signals_ended() -> void:
	...
	# Signal should be emitted
	assert_signal_emitted(infighting_system, "infight_ended", "Infighting end should emit signal")
...
func test_infighting_force_end() -> void:
	...
	assert_false(infighting_system.is_infighting(), "Force end should stop infighting")
...
func test_infighting_extend_duration() -> void:
	...
	assert_true(
		infighting_system.is_infighting(), "Infighting should continue after extending duration"
	)
...
func test_infighting_disabled_globally() -> void:
	...
	# Should NOT start infighting
	assert_false(infighting_system.is_infighting(), "Infighting should not start when disabled")
...
func test_infighting_no_self_attack() -> void:
	...
	assert_false(infighting_system.is_infighting(), "Enemy should not infight with itself")
...
func test_infighting_target_retrieval() -> void:
	...
	assert_eq(target, attacker, "Should return correct infight target")
...
func test_infighting_notify_damage_from() -> void:
	...
	# Should start infighting
	assert_true(infighting_system.is_infighting(), "notify_damage_from should trigger infighting")
...
func test_infighting_retaliation_chance_zero() -> void:
	...
	assert_false(
		infighting_system.is_infighting(), "Infighting should not start with 0% retaliation chance"
	)
...
func test_infighting_duration_expires() -> void:
	...
	# Should no longer be infighting
	assert_false(infighting_system.is_infighting(), "Infighting should end after duration expires")
...
func test_infighting_only_targets_enemies() -> void:
	...
	assert_false(
		infighting_system.is_infighting(), "Infighting should only target enemies, not players"
	)
...
func test_movement_component_exists() -> void:
	...
	assert_not_null(movement, "Enemy should have movement component for pathfinding")
...
func test_movement_component_has_navigation_agent() -> void:
	...
	assert_true("nav_agent" in movement, "Movement component should have NavigationAgent3D")

	if "nav_agent" in movement:
		assert_not_null(movement.nav_agent, "NavigationAgent3D should be instantiated")
...
func test_navigation_agent_configuration() -> void:
	...
	assert_gte(nav_agent.path_desired_distance, 0.0, "Path desired distance should be non-negative")
	assert_gte(
		nav_agent.target_desired_distance, 0.0, "Target desired distance should be non-negative"
	)
...
func test_ai_can_set_target_position() -> void:
	...
	if movement.nav_agent:
		assert_eq(
			movement.nav_agent.target_position, target_pos, "Navigation agent target should be set"
		)
...
func test_chase_state_sets_target_position() -> void:
	...
	assert_true(true, "Chase state should use pathfinding to reach target")
...
func test_pathfinding_around_obstacles() -> void:
	...
	assert_true(true, "Pathfinding should navigate around obstacles")
...
func test_pathfinding_velocity_calculation() -> void:
	...
	assert_true(
		"speed" in movement, "Movement component should have speed property for pathfinding"
	)
...
func test_pathfinding_rotation_toward_target() -> void:
	...
	assert_true(
		"rotation_speed" in movement, "Movement component should have rotation_speed for turning"
	)
...
func test_pathfinding_with_squad_formation() -> void:
	...
	assert_true(true, "Pathfinding should integrate with squad formation positions")
...
func test_perception_component_exists() -> void:
	assert_not_null(
		perception_component, "Enemy should have perception component for target selection"
	)
...
func test_perception_has_detection_radius() -> void:
	...
	assert_true(
		"detection_radius" in perception_component, "Perception should have detection radius"
	)

	if "detection_radius" in perception_component:
		assert_gt(perception_component.detection_radius, 0.0, "Detection radius should be positive")
...
func test_perception_signals_target_lost() -> void:
	...
	assert_true(
		perception_component.has_signal("target_lost"), "Perception should have target_lost signal"
	)
...
func test_ai_controller_has_target_property() -> void:
	...
	assert_true("target" in ai_controller, "AI controller should have target property")
...
func test_target_selection_on_damage() -> void:
	...
	if ai_controller.has_method("on_damage_received"):
		...
		assert_eq(ai_controller.target, attacker, "AI should target attacker on damage")
...
func test_target_priority_closest_threat() -> void:
	...
	if ai_controller and ai_controller.target:
		...
		assert_lt(target_dist, 10.0, "AI should prioritize closer target")
...
func test_target_selection_distance_based() -> void:
	...
	if ai_controller.target:
		assert_eq(ai_controller.target, target, "AI should select target within vision range")
...
func test_target_selection_ignores_out_of_range() -> void:
	...
	assert_null(ai_controller.target, "AI should not select targets outside vision range")
...
func test_target_selection_switches_on_damage() -> void:
	...
	assert_eq(ai_controller.target, attacker, "AI should switch target to attacker on damage")
...
func test_target_selection_ignores_dead_targets() -> void:
	...
	assert_null(ai_controller.target, "AI should not target dead entities")
...
func test_target_selection_ignores_invisible_targets() -> void:
	...
	assert_null(ai_controller.target, "AI should not target invisible entities")
...
func test_target_selection_multiple_targets() -> void:
	...
	if ai_controller.target:
		assert_true(
			ai_controller.target in targets, "AI should select one of the available targets"
		)
...
func test_target_selection_prefers_players_over_enemies() -> void:
	...
	if ai_controller.target:
		assert_eq(
			ai_controller.target, player_target, "AI should prefer player targets over enemies"
		)
...
func test_target_selection_no_self_targeting() -> void:
	...
	assert_ne(ai_controller.target, enemy, "AI should never target itself")
...
func test_target_selection_clears_on_target_lost() -> void:
	...
	assert_null(ai_controller.target, "AI target should be cleared when target_lost signal emits")
...
func test_target_selection_threat_level_by_distance() -> void:
	...
	if ai_controller.target:
		...
		assert_lt(target_dist, 5.0, "AI should prioritize closer threats")
...
func test_target_selection_retains_valid_target() -> void:
	...
	assert_eq(ai_controller.target, initial_target, "AI should retain valid target across scans")
...
func test_target_selection_switches_to_closer_target() -> void:
	...
	if ai_controller.target:
		...
		assert_lt(target_dist, 8.0, "AI should switch to closer target when available")
...
func test_target_selection_validates_target_instance() -> void:
	...
	assert_eq(ai_controller.target, new_attacker, "AI should handle invalid targets gracefully")
...
func test_combat_component_exists() -> void:
	assert_not_null(combat_component, "Enemy should have combat component")
...
func test_combat_has_attack_damage() -> void:
	...
	assert_true("attack_damage" in combat_component, "Combat component should have attack damage")
...
func test_combat_has_attack_range() -> void:
	...
	assert_true("attack_range" in combat_component, "Combat component should have attack range")
...
func test_ai_config_loaded_from_data() -> void:
	...
	assert_true(grunt_data.has("ai_config"), "Enemy data should have ai_config section")
...
func test_ai_can_be_disabled() -> void:
	...
	assert_true("is_ai_active" in enemy, "Enemy should have is_ai_active flag")
