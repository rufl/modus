...
const SAVE_SLOTS: Array[String] = ["gut_progression_save", "gut_progression_load", "gut_progression_roundtrip"]
...
func test_progression_persistence_roundtrip() -> void:
	...
	var expected := [progression.get_level(), progression.get_current_xp(), progression.skill_points]
	progression.reset_progression()
	assert_true(progression.load_progression(SAVE_SLOTS[2]))
	assert_eq([progression.get_level(), progression.get_current_xp(), progression.skill_points], expected)
