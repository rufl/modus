...
func test_validate_config_valid():
	var config := {"max_health": 100, "player_name": "Test", "settings": {"volume": 0.8}}

	var schema := {
		"max_health": TYPE_INT, "player_name": TYPE_STRING, "settings.volume": TYPE_FLOAT
	}
...
func test_validate_config_missing_key():
	var config := {"max_health": 100}

	var schema := {"max_health": TYPE_INT, "player_name": TYPE_STRING}
...
func test_validate_config_wrong_type():
	var config := {"max_health": "not_a_number"}

	var schema := {"max_health": TYPE_INT}
...
func test_path_resolution_with_res_prefix():
	var config: Dictionary = config_manager.load_config_file(
		"res://tests/fixtures/config/test_valid.json5"
	)
