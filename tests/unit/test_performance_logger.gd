...
func test_logging_signals() -> void:
	...
	logger.logging_started.connect(func(session: String) -> void:
		received.started = true
		received.session_name = session
	)

	logger.logging_stopped.connect(func(_session: String, path: String) -> void:
	...
	assert_string_contains(received.file_path, session_name, "Signal should contain correct file path")
