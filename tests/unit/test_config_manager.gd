...
func test_get_value() -> void:
	...
	if gm and gm.get_core_system("config"):
		...
		var result: Variant = gm.get_core_system("config").get_value(
			"nonexistent.path", fallback_value
		)
...
func test_get_config() -> void:
	...
	if gm and gm.get_core_system("config"):
		...
		if player_config != null:
			assert_eq(
				typeof(player_config),
				TYPE_DICTIONARY,
				"Player config should be dictionary when present"
			)
		else:
