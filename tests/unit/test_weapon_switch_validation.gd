...
func test_request_weapon_switch_in_whitelist() -> void:
	...
	assert_true(
		allowed_rpcs.has("request_weapon_switch"),
...
func test_request_weapon_switch_requires_validation() -> void:
	...
	assert_true(
		rpc_config.get("requires_validation", false),
		"request_weapon_switch should require server validation"
	)

	# Assert: Has validator method
	assert_true(
		rpc_config.has("validator_method"), "request_weapon_switch should have a validator method"
	)
...
func test_request_weapon_switch_rate_limit() -> void:
	...
	assert_eq(rate_limit, 5.0, "request_weapon_switch should have 5 calls/sec rate limit")
...
func test_rapid_weapon_switches_respect_rate_limit() -> void:
	...
	assert_gt(allowed_calls, 0, "Some weapon switches should be allowed within rate limit")
...
func test_weapon_switch_rpc_has_description() -> void:
	...
	assert_true(rpc_config.has("description"), "request_weapon_switch should have description")

	var description: String = rpc_config.get("description", "")
	assert_false(description.is_empty(), "request_weapon_switch description should not be empty")


func test_confirm_weapon_switch_in_whitelist() -> void:
	# Assert: Server confirmation RPC exists
	assert_true(
		allowed_rpcs.has("confirm_weapon_switch"),
...
func test_reject_weapon_switch_in_whitelist() -> void:
	...
	assert_true(
		allowed_rpcs.has("reject_weapon_switch"),
...
func test_reject_weapon_switch_rate_limit() -> void:
	...
	assert_true(rpc_config.has("calls_per_second"), "reject_weapon_switch should have rate limit")

	var rate_limit: float = rpc_config.get("calls_per_second", 0.0)
	assert_eq(
		rate_limit, 5.0, "reject_weapon_switch should have same rate limit as request (5 calls/sec)"
	)
...
func test_weapon_switch_validation_flow() -> void:
	...
	assert_true(
		allowed_rpcs.has("request_weapon_switch"), "Step 1: Client can send request_weapon_switch"
	)

	# 2. Server validates the request
	var request_config: Dictionary = allowed_rpcs.get("request_weapon_switch", {})
	assert_true(
		request_config.get("requires_validation", false), "Step 2: Server validates the request"
	)

	# 3. Server can confirm or reject
	assert_true(
		allowed_rpcs.has("confirm_weapon_switch"), "Step 3a: Server can confirm valid switch"
	)
	assert_true(
		allowed_rpcs.has("reject_weapon_switch"), "Step 3b: Server can reject invalid switch"
	)
...
func test_rate_limit_prevents_spam() -> void:
	...
	assert_eq(expected_blocks, 95, "Rate limiting should block 95 out of 100 spam attempts")

	# Assert: Rate limit is effective
	var block_percentage: float = (float(expected_blocks) / float(attack_rate)) * 100.0
	assert_gt(block_percentage, 90.0, "Rate limiting should block >90% of spam attempts")
...
func test_weapon_switch_does_not_require_authentication() -> void:
	...
	assert_true(
		rpc_config.get("requires_validation", false), "Requires validation for rate limiting"
	)
...
func test_all_weapon_rpcs_have_rate_limits() -> void:
	...
	var weapon_rpcs: Array[String] = [
		"request_weapon_switch", "confirm_weapon_switch", "reject_weapon_switch", "request_reload"
	]
...
func test_network_manager_validates_weapon_switch() -> void:
	...
	assert_false(
		network_manager.validate_rpc(test_peer_id, rpc_name, []),
	...
	assert_true(
		network_manager._check_rate_limit(rate_limit_peer_id, rpc_name),
