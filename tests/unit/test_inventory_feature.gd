...
func test_inventory_data_signals():
	...
	inventory.item_added.connect(func(_item, _slot):
		signals_received["item_added"] += 1
	)
	inventory.item_removed.connect(func(_item, _slot):
		signals_received["item_removed"] += 1
	)
