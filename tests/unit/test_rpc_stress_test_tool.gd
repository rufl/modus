...
class MockRPCStressTest extends Node:
...
func test_different_rate_limits() -> void:
	...
	for i in range(total_calls):
		var allowed: bool = test_instance._simulate_rpc_call("test_rpc", rate_limit_5, i, total_calls)
		if not allowed:
			violations_5 += 1
	...
	assert_almost_eq(
		...
		45.0,
		5.0,
		"Should detect ~45 violations with 5 calls/sec limit"
	)
	...
	for i in range(total_calls):
		var allowed: bool = test_instance._simulate_rpc_call("test_rpc", rate_limit_20, i, total_calls)
		if not allowed:
			violations_20 += 1
	...
	assert_almost_eq(
		...
		30.0,
		5.0,
		"Should detect ~30 violations with 20 calls/sec limit"
	)
...
func test_high_rate_limit() -> void:
	...
	assert_almost_eq(
		...
		5.0,
		3.0,
		"Should detect ~5 violations with 95 calls/sec limit"
	)
...
func test_rpc_whitelist_loading() -> void:
	...
	if rpc_whitelist_script:
		...
		assert_true(
			allowed_rpcs.has("request_shoot"),
			"RPC whitelist should contain request_shoot"
		)
...
func test_integration_test_class_exists() -> void:
	...
	assert_string_contains(
		...
		"Script should contain IntegrationTest inner class"
	)
	assert_string_contains(
		script_source,
