...
func test_revive_within_distance_threshold_allowed() -> void:
	...
	assert_lte(
		distance,
		REVIVE_DISTANCE_THRESHOLD,
		(
			"Reviver should be within distance threshold (%.1fm <= %.1fm)"
			% [distance, REVIVE_DISTANCE_THRESHOLD]
		)
	)
...
func test_revive_beyond_distance_threshold_rejected() -> void:
	...
	assert_gt(
		...
		REVIVE_DISTANCE_THRESHOLD,
		(
			"Reviver should be beyond distance threshold (%.1fm > %.1fm)"
			% [distance, REVIVE_DISTANCE_THRESHOLD]
		)
	)
...
func test_revive_exactly_at_threshold_allowed() -> void:
	...
	assert_almost_eq(
		distance, REVIVE_DISTANCE_THRESHOLD, 0.01, "Reviver should be exactly at distance threshold"
	)
...
func test_revive_distance_validation_3d_space() -> void:
	...
	assert_almost_eq(distance, expected_distance, 0.01, "Distance should be calculated in 3D space")
...
func test_revive_distance_validation_vertical_distance() -> void:
	...
	assert_gt(distance, REVIVE_DISTANCE_THRESHOLD, "Vertical distance should be properly validated")
...
func test_revive_distance_negative_coordinates() -> void:
	...
	assert_almost_eq(
		distance, 1.0, 0.01, "Distance should be calculated correctly with negative coordinates"
	)

	# Assert: Within threshold
	assert_lte(distance, REVIVE_DISTANCE_THRESHOLD, "Revive should work with negative coordinates")
