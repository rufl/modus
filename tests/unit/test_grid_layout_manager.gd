	...
	for y in range(64):
		for x in range(64):
			...
			assert_eq(
				cell.type,
				Cell.Type.EMPTY,
				"Cell at (%d, %d) should be EMPTY" % [x, y]
			)
...
func test_is_valid_position() -> void:
	...
	assert_true(grid_manager.is_valid_position(Vector2i(63, 63)), "Bottom-right corner should be valid")
...
func test_get_neighbors_4_center_position() -> void:
	...
	for expected_pos in expected:
		assert_true(
			neighbors.has(expected_pos),
			"Should include neighbor at %s" % expected_pos
		)
...
func test_flood_fill_connected_region() -> void:
	...
	var filter_func := func(cell: Cell) -> bool:
		return cell.type == Cell.Type.ROOM
...
func test_is_walkable_cell() -> void:
	...
	for cell_type in walkable_types:
		...
		assert_true(
			grid_manager.is_walkable_cell(cell),
			"Cell type %s should be walkable" % cell_type
		)
	
	var empty_cell := Cell.new(Cell.Type.EMPTY)
	assert_false(
		grid_manager.is_walkable_cell(empty_cell),
		"EMPTY cell should not be walkable"
	)
