...
func test_graphics_settings_apply_medium_preset() -> void:
	...
	assert_eq(
		current_settings.get("name", ""),
		"Medium",
		"Medium preset should be applied"
	)
...
func test_graphics_settings_apply_ultra_preset() -> void:
	...
	assert_eq(
		current_settings.get("name", ""),
		"Ultra",
		"Ultra preset should be applied"
	)
...
func test_quality_changed_signal_emitted_on_preset_change() -> void:
	...
	assert_signal_emitted(graphics_system, "quality_changed", "quality_changed signal should emit on preset change")
...
func test_low_quality_has_minimal_features() -> void:
	...
	var current_settings: Dictionary = graphics_system.current_settings
	assert_false(
		current_settings.get("shadow_enabled", true),
		"Low preset should disable shadows"
	)
	assert_false(
		current_settings.get("ssao_enabled", true),
		"Low preset should disable SSAO"
	)
	assert_false(
		current_settings.get("ssr_enabled", true),
		"Low preset should disable SSR"
	)
	assert_false(
		current_settings.get("volumetric_fog", true),
		"Low preset should disable volumetric fog"
	)
...
func test_medium_quality_has_balanced_features() -> void:
	...
	var current_settings: Dictionary = graphics_system.current_settings
	assert_true(
		current_settings.get("shadow_enabled", false),
		"Medium preset should enable shadows"
	)
	assert_true(
		current_settings.get("glow_enabled", false),
		"Medium preset should enable glow"
	)
	assert_false(
		current_settings.get("ssao_enabled", true),
		"Medium preset should disable SSAO"
	)
...
func test_high_quality_has_advanced_features() -> void:
	...
	assert_true(
		current_settings.get("shadow_enabled", false),
		"High preset should enable shadows"
	)
	assert_true(
		current_settings.get("ssao_enabled", false),
		"High preset should enable SSAO"
	)
	assert_true(
		current_settings.get("volumetric_fog", false),
		"High preset should enable volumetric fog"
	)
...
func test_ultra_quality_has_maximum_features() -> void:
	...
	assert_true(
		current_settings.get("shadow_enabled", false),
		"Ultra preset should enable shadows"
	)
	assert_true(
		current_settings.get("ssao_enabled", false),
		"Ultra preset should enable SSAO"
	)
	assert_true(
		current_settings.get("ssr_enabled", false),
		"Ultra preset should enable SSR"
	)
	assert_true(
		current_settings.get("sdfgi_enabled", false),
		"Ultra preset should enable SDFGI"
	)
...
func test_shader_compilation_succeeds_for_all_presets() -> void:
	...
	for preset: String in presets:
		...
		assert_not_null(
			...
			"Graphics system should remain functional with %s preset (shader compilation succeeded)" % preset
		)
...
func test_max_particles_varies_by_quality() -> void:
	...
	assert_gt(
		ultra_particles,
		low_particles,
		"Ultra preset should allow more particles than low preset"
	)
...
func test_lod_bias_varies_by_quality() -> void:
	...
	assert_lt(
		high_lod_bias,
		low_lod_bias,
		"High preset should have lower LOD bias than low preset"
	)
