		...
		for y in range(region.position.y, region.end.y):
			for x in range(region.position.x, region.end.x):
				if context.grid[y][x].type == Cell.Type.OUTDOOR:
					...
					assert_eq(context.grid[y][x].metadata.get("has_ceiling", true), false,
						"Outdoor cells should have has_ceiling = false")
					assert_eq(context.grid[y][x].metadata.get("is_outdoor", false), true,
...
func test_outdoor_area_count_scales_with_map_size() -> void:
	...
	assert_lte(small_areas.size(), large_areas.size(),
		"Larger maps should have same or more outdoor areas")
...
func test_outdoor_bias_scales_frequency() -> void:
	...
	assert_gt(high_bias_count, low_bias_count,
		"Higher bias should generate more outdoor cells")
