...
func test_generate_room_shape_small() -> void:
	...
	var points: PackedVector2Array = engine.generate_room_shape(center, target_size, room_type, rng, null)
...
func test_polygon_to_grid_cells() -> void:
	...
	for cell in cells:
		assert_true(
			cell.x >= 0 and cell.x < context.grid_size.x,
			"Cell X should be in bounds"
		)
		assert_true(
			cell.y >= 0 and cell.y < context.grid_size.y,
			"Cell Y should be in bounds"
		)
...
func test_generate_room_creates_valid_room() -> void:
	...
	assert_gt(room.entrance_points.size(), 0, 
		"Room should have at least one entrance point")
...
func test_room_meets_minimum_size_requirements() -> void:
	...
	# Test small room
	var small_room: Room = engine.generate_room(
		center, Room.RoomType.SMALL, 0, context
	)
	...
	assert_true(medium_room.cells.size() >= 9, 
		"Medium room should have at least 9 cells")
	
	# Test large room
	var large_room: Room = engine.generate_room(
		center + Vector2i(40, 0), Room.RoomType.LARGE, 2, context
	)
	assert_true(large_room.cells.size() >= 17, 
		"Large room should have at least 17 cells")


func test_load_theme_rules_default() -> void:
	var rules: Dictionary = engine.load_theme_rules("")
	assert_eq(rules, engine.DEFAULT_RULES, 
		"Empty theme should return default rules")
...
func test_generate_rooms_creates_multiple_rooms() -> void:
	...
	assert_true(rooms.size() <= room_count, 
		"Should not generate more rooms than requested")
	...
	for room in rooms:
		...
		assert_gt(room.cells.size(), 0, "Room should have cells")
		assert_gt(room.entrance_points.size(), 0, 
			"Room should have entrance points")


func test_apply_theme_modifications() -> void:
	var room: Room = engine.generate_room(
		Vector2i(64, 64), Room.RoomType.MEDIUM, 0, context
	)
