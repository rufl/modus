...
func test_key_locations_marked_in_metadata() -> void:
	...
	for key: Dictionary in keys:
		...
		if grid_pos != Vector2i(-1, -1) and grid_pos.x >= 0 and grid_pos.x < context.grid_size.x and \
		   grid_pos.y >= 0 and grid_pos.y < context.grid_size.y:
			var cell: Cell = context.grid[grid_pos.y][grid_pos.x]
			assert_true(cell.metadata.get("has_key", false), "Key cell should be marked in metadata")
