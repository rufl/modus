...
func test_fake_wall_marked_in_metadata() -> void:
	...
	if secret_rooms.size() > 0:
		...
		assert_true(
			wall_cell.metadata.get("is_fake_wall", false), "Fake wall should be marked in metadata"
		)
...
func test_secret_cells_marked_as_secret_type() -> void:
	...
	if secret_rooms.size() > 0:
		...
		for cell_pos: Vector2i in secret_room.cells:
			...
			assert_eq(
				cell.type, Cell.Type.SECRET, "Secret room cells should be marked as SECRET type"
			)
			assert_true(
				cell.metadata.get("is_secret", false),
				"Secret room cells should have is_secret metadata"
			)
...
func test_secret_rooms_marked_for_achievement_tracking() -> void:
	...
	if secret_rooms.size() > 0:
		...
		assert_true(
			secret_room.has("fake_wall_position"), "Secret room should have fake wall position"
		)
