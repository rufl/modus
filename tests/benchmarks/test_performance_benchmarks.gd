...
func test_memory_leak_detection() -> void:
	...
	for cycle in range(5):
		...
		print("Cycle %d: Memory = %.2f MB, Objects = %d" % [
			cycle + 1,
			current_static / 1024.0 / 1024.0,
			current_objects
		])
...
func test_network_performance_under_load() -> void:
	...
	print("Bandwidth - Avg In: %.2f KB/s, Avg Out: %.2f KB/s (simulated)" % [
		network_stats.avg_in_bandwidth_kb,
		network_stats.avg_out_bandwidth_kb
	])
	print("Packet Rate - Avg In: %.1f/s, Avg Out: %.1f/s (simulated)" % [
		packet_stats.avg_packets_in_per_sec,
		packet_stats.avg_packets_out_per_sec
	])
	print("Latency - Avg: %.1f ms, Jitter: %.1f ms (simulated)" % [
		latency_stats.avg_latency_ms,
		latency_stats.jitter_ms
	])
