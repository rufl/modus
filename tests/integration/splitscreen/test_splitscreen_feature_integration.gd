...
func test_feature_initialization() -> void:
	...
	assert_not_null(splitscreen_manager.viewport_manager, "ViewportManager should be created")
	assert_not_null(splitscreen_manager.gamepad_controller, "GamepadController should be created")
	assert_not_null(splitscreen_manager.session_state, "SessionState should be created")
...
func test_feature_loading() -> void:
	...
	assert_true(
		is_instance_valid(splitscreen_manager), "SplitscreenManager should be valid after loading"
	)
...
func test_feature_unloading() -> void:
	...
	assert_false(
		splitscreen_manager.is_session_active(), "Session should not be active after unloading"
	)
...
func test_feature_toggle_configuration() -> void:
	...
	assert_ge(splitscreen_manager.min_players, 4, "Min players should be configured")
	assert_le(splitscreen_manager.max_players, 6, "Max players should be configured")
...
func test_feature_isolation_when_disabled() -> void:
	...
	assert_null(
		splitscreen_manager.viewport_manager, "ViewportManager should not exist when disabled"
	)
	assert_null(
		splitscreen_manager.gamepad_controller, "GamepadController should not exist when disabled"
	)
...
func test_multiple_load_unload_cycles() -> void:
	for i in range(3):
		splitscreen_manager.initialize()
		assert_not_null(splitscreen_manager.session_state, "Should initialize on cycle %d" % i)
...
func test_configuration_loading_from_file() -> void:
	...
	assert_true(splitscreen_manager.config.size() > 0, "Configuration should be loaded")
...
func test_default_configuration_when_file_missing() -> void:
	...
	assert_eq(splitscreen_manager.min_players, 4, "Should use default min_players")
	assert_eq(splitscreen_manager.max_players, 6, "Should use default max_players")
...
func test_feature_dependencies() -> void:
	...
	assert_not_null(
		splitscreen_manager.viewport_manager, "ViewportManager dependency should be met"
	)
	assert_not_null(
		splitscreen_manager.gamepad_controller, "GamepadController dependency should be met"
	)
...
func test_feature_reinitialization() -> void:
	...
	assert_not_null(splitscreen_manager.viewport_manager, "Should create new ViewportManager")
	assert_ne(
		splitscreen_manager.viewport_manager, first_viewport_manager, "Should be a new instance"
	)
