...
func test_splitscreen_disabled_no_network_impact() -> void:
	...
	assert_null(test_manager.viewport_manager,
		"Splitscreen components should not exist when disabled")
	assert_null(test_manager.gamepad_controller,
		"Splitscreen components should not exist when disabled")
	test_manager.free()
...
func test_input_isolation_splitscreen_network() -> void:
	...
	assert_eq(input_comp.get_device_id(), 0,
		"Input should be isolated to assigned device")
...
func test_viewport_network_independence() -> void:
	...
	assert_eq(splitscreen_manager.viewport_manager.get_viewport_count(), 4,
...
func test_gamepad_assignment_network_independence() -> void:
	...
	assert_eq(splitscreen_manager.gamepad_controller.get_assigned_device(0), 0,
...
func test_error_handling_isolation() -> void:
	...
	assert_true(is_instance_valid(network_manager),
		"Network manager should survive splitscreen errors")
...
func test_feature_toggle_network_independence() -> void:
	...
	assert_true(is_instance_valid(network_manager),
		"Network manager unaffected by splitscreen toggle")
...
func test_cleanup_network_independence() -> void:
	...
	assert_true(is_instance_valid(network_manager),
		"Network resources unaffected by splitscreen cleanup")
...
func test_configuration_independence() -> void:
	...
	assert_true(splitscreen_manager.config.size() > 0,
		"Splitscreen config should be loaded")
...
func test_concurrent_sessions() -> void:
	...
	assert_true(splitscreen_manager.is_session_active(),
		"Splitscreen session should be active")
	assert_true(network_active,
		"Network session should be active")
